...
--  if two records have the same price, we can then apply a second ordering rule.
SELECT * FROM products ORDER BY price, weight;

-- ascending price and descending weight
-- We can apply sort order options to each of these different properties or these different column names.
SELECT * FROM products ORDER BY price, weight DESC;

-- skip the first 40 rows and return rest
-- We make use of offset any time that we want to skip some number of records in a result set.
